...
def def_four_lines(X, X_er, Y, Y_er):
    if Y + Y_er < stas06(X) and X + X_er < -0.3:
        AGN = 'SFGXY'
...
def four_lines(X, X_er, pair_x_flags, Y, Y_er, pair_y_flags):
    ...
    if X - X_er > 0.47 and Y + Y_er < 1.19:
        if pair_x_flag == 'left':
            ...
            AGN = 'AGNXY'
    elif X + X_er < 0.47 and Y - Y_er > 0.87:
        if pair_y_flag == 'down':
            AGN = 'UNCXY'
        else:
            AGN = 'AGNXY' 
...
def AGN_reg(OIII, OIII_er, HB, HB_er, NII, NII_er, HA, HA_er, HA_ew, HA_ew_err, z):
    ...
    if X == -100 or Y == -100:
        ...
        AGN = double_line(Y, Y_er, pair_y_flags, 0.87, 1.19, 'Y')
        SC_WHAN = WHAN(X, X_er, pair_x_flags, HA_ew, HA_ew_err, pair_HA)
    elif X != 99 and Y == -99:
        AGN = double_line(X, X_er, pair_x_flags, -0.3, 0.47, 'X')
        # AGN = double_line(X, X_er, pair_x_flags, -0.4, -0.2, 'X')
        SC_WHAN = WHAN(X, X_er, pair_x_flags, HA_ew, HA_ew_err, pair_HA)
