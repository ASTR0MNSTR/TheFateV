...
class Main:
    ...
    def merging_WHAN(self, list_obj):
        ...
        sAGN, wAGN, UNC, SF, ELR, LLR, NER = list_obj
    ...
    def histo(self, figure, keys, kwarg):
        size = 0.45
        merged_WHAN = Main.merging_WHAN(self, Main.sorting_forWHAN(self, keys))
        merged_BPT = Main.merging_BPT(self, Main.sorting_forBPT(self, keys))
        if kwarg == 'WHAN':
            ...
            figure.pie(merged_WHAN, radius=1-size, colors=self.WHAN_colors_merged, autopct=Main.short_WHAN_in(Main.merging_WHAN(self, Main.sorting_forWHAN(self, keys))), wedgeprops=dict(width=size, edgecolor='w'))
            ...
            figure.pie(merged_BPT, radius=1-size, colors=self.BPT_colors_merged, autopct=Main.short_BPT_in(Main.merging_BPT(self, Main.sorting_forBPT(self, keys))), wedgeprops=dict(width=size, edgecolor='w'))
            figure.set(aspect='equal')
