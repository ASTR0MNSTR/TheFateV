...
class Main(hp):
    ...
    def plotting_BPT_presentation(self):
        self.gs_top = plt.GridSpec(1, 1, wspace=0, hspace=0)
        self.fig = plt.figure(figsize=(12, 12), tight_layout=True)
        # adjusting_figure_size(1, 12, l=1.2, r=1.8, b=0.6, t=0.3)
        adjusting_plotting_pars()
        k = 0
        ...
        for ax in self.topaxes:    
            ...
            ax.text(0.5, -0.5, 'LINER', weight='bold')
            ax.set_box_aspect(1)
        
        self.ax4.set_xlabel(r"$\log \mathrm{([NII]/H\alpha)}$")
        self.ax4.set_ylabel(r'$\log{\mathrm{([OIII]/H\beta)}}$')
        ...
        for pars in self.flux_er_mod9:
            ...
            if x != -100 and x != -99 and y != -99 and y != 100:
                ...
                if len(pair_x_flags) == 0 and len(pair_y_flags) == 0:
                    try:
                        self.ax4.scatter(
                            x, y, s=60, color=self.color_dict[AGN][0], alpha=1)
                    #self.ax4.scatter(x, y, s=1.5, color=self.s_m.to_rgba(age), alpha=1)
                        # self.ax5.scatter(x, y, s=self.cd_WHAN[SC_WHAN][1], color=self.cd_WHAN[SC_WHAN][0], marker =self.cd_WHAN[SC_WHAN][2], alpha=1)
                        #self.ax4.scatter(x, y, s=self.cd_WHAN[SC_WHAN][1], color=self.cd_WHAN[SC_WHAN][0], marker =self.cd_WHAN[SC_WHAN][2], alpha=0.5)
                        #self.ax5.scatter(x, y, s=self.cd_WHAN[SC_WHAN][1], color=self.s_m.to_rgba(age), marker =self.cd_WHAN[SC_WHAN][2], alpha=0.5)
                    ...
                    #Main.plotting_arrows(self, self.ax4, x, y, pair_x_flags, pair_y_flags, self.cd_WHAN[SC_WHAN][0], m_x = 1, m_y = 1)
                    #Main.plotting_arrows(self, self.ax5, x, y, pair_x_flags, pair_y_flags, self.s_m.to_rgba(age), m_x = 1, m_y = 1)
                    Main.plotting_arrows(self, self.ax4, x, y, pair_x_flags, pair_y_flags, self.color_dict[AGN][0], m_x = 1, m_y = 1, alpha=1)
                    # Main.plotting_arrows(self, self.ax_med_BPT, x, y, pair_x_flags, pair_y_flags, self.s_m.to_rgba(age), m_x = 0.7, m_y = 0.7, alpha=0.5)
        ...
        # for key in BPT_color_plt:
        #     self.ax_med_BPT.scatter(-99, -99, alpha=1, color = BPT_color_plt[key][0], s = BPT_color_plt[key][1], marker= BPT_color_plt[key][2], label=key)
        self.ax4.scatter(-99, -99, alpha= 1, color = 'midnightblue', label='AGNXY', s = 150)
        # self. ax4.scatter(-99, -99, alpha= 1, color = 'dodgerblue', label='AGNX', s = 30, marker='o')
        self.ax4.scatter(-99, -99, alpha= 1, color = 'springgreen', label='UNCXY', s = 150)
        # self.ax4.scatter(-99, -99, alpha= 1, color = 'darkgreen', label='UNCX', s = 30, marker='o')
        self.ax4.scatter(-99, -99, alpha= 1, color = 'mediumvioletred', label='SFGXY', s = 150)
        # self.ax4.scatter(-99, -99, alpha= 1, color = 'crimson', label='SFGX', s = 30, marker='o')
        self.ax4.legend(loc=3)
        print('Number of points on BPT: ', k)
        self.fig.savefig('BPT.pdf')
    
    def plotting_WHAN_presentation(self):
        self.gs_top = plt.GridSpec(1, 1, wspace=0, hspace=0)
        self.fig = plt.figure(figsize=(12, 12), tight_layout=True)
        # adjusting_figure_size(1, 12, l=1.2, r=1.8, b=0.6, t=0.3)
        adjusting_plotting_pars()

        self.ax7 = self.fig.add_subplot(self.gs_top[0,0])
        ...
        self.ax7.tick_params(top=True, labeltop=False, bottom=True, labelbottom=True, right=True, labelleft=True, left=True, direction='in')
        
        self.topaxes=[self.ax7]
        #for ax in self.topaxes[1:]:
        #plt.setp(ax.get_xticklabels(), visible=False)
        ...
        for ax in self.topaxes:
            ...
            ax.set_xlim([-1.9, 1.2])
            ax.set_ylim([-2, 2.7])
            ax.axhline(y = 0.47712, color = 'black', linestyle='dashed', linewidth=5)
            ax.axhline(y = -0.301, color = 'black', linestyle='dotted', linewidth=5)
            ax.text(0.8, 0, 'ELR', weight='bold')
            ax.text(0.8, -1.5, 'LLR', weight='bold')

            X_wAGN = np.arange(-0.4, 2.5, 0.01)
            ax.plot(X_wAGN, 0.77815125+X_wAGN*0, 'black', linewidth=5)
            ax.text(-1.5, 2, 'SF', weight='bold')
            ax.text(0.6, 0.5, 'wAGN', weight='bold')
            ax.text(0.6, 2, 'sAGN', weight='bold')

            Y_sAGN = np.arange(0.47712, 3, 0.01)
            ax.plot(-0.4+Y_sAGN*0, Y_sAGN, 'black', linewidth=5)
            ax.set_xlabel(r"$\log \mathrm{([NII]/H\alpha)}$")
            ax.set_box_aspect(1)
        

        self.ax7.set_ylabel(r"$\log \mathrm{(EW_{\mathrm{H\alpha}})}$")
        ...
        for pars in self.flux_er_mod9:
            ...
            if x != -100 and x != -99 and y != -99 and y != 100:
                ...
                if len(pair_x_flags) == 0 and len(pair_y_flags) == 0:
                    # self.ax6.scatter(x, y, s=self.color_dict[AGN][1], color=self.color_dict[AGN][0], alpha=1, marker=self.color_dict[AGN][2])
                    # self.ax_med_WHAN.scatter(x, y, s=self.color_dict[AGN][1], color=self.s_m.to_rgba(age), alpha=0.3, marker=self.color_dict[AGN][2])
                    self.ax7.scatter(x, y, s=60, color=self.cd_WHAN[SC_WHAN][0], marker = '.', alpha=1)
                    #self.ax6.scatter(x, y, s=self.cd_WHAN[SC_WHAN][1], color=self.cd_WHAN[SC_WHAN][0], marker = '.', alpha=0.5)
                    #self.ax7.scatter(x, y, s=self.cd_WHAN[SC_WHAN][1], color=self.s_m.to_rgba(age), marker = '.', alpha=0.5)
            
                else:
                    Main.plotting_arrows(self, self.ax7, x, y, pair_x_flags, pair_y_flags, self.cd_WHAN[SC_WHAN][0], m_y = 1.2, m_x = 1, alpha=1)
                    # Main.plotting_arrows(self, self.ax_med_WHAN, x, y, pair_x_flags, pair_y_flags, self.s_m.to_rgba(age), m_y = 1, m_x = 0.8, alpha=0.5)
                    # Main.plotting_arrows(self, self.ax6, x, y, pair_x_flags, pair_y_flags, self.color_dict[AGN][0], m_y = 1.2, m_x = 1, alpha = 1)
                    # Main.plotting_arrows(self, self.ax6, x, y, pair_x_flags, pair_y_flags, self.cd_WHAN[SC_WHAN][0], m_y = 1, m_x = 0.8)
                    
        class_list = class_list_creator_wo_err(X, Y, AGE, AGN_flags, 'WHAN')

        for key in self.cd_WHAN_leg.keys():
            self.ax7.scatter(-99, -99, alpha= 1, color = self.cd_WHAN_leg[key][0], marker = self.cd_WHAN_leg[key][2], s = 150, label=key)
...
if __name__ == '__main__':
    ...
    obj.gama_reading()
    obj.sorting()
    obj.plotting_BPT_presentation()
    obj.plotting_WHAN_presentation()
    # obj.plotting_WHAN()
    #obj.plotting_EW_age()