...
class Main(hp):
    ...
    def reading(self):
        ...
        for line in lines_stripped:
            ...
            Y = float(line.split()[35])
            Y_up = float(line.split()[36])
            Y_down = float(line.split()[34])
    ...
    def plotting_mdms_age(self):
        ...
        self.ax4.set_ylabel(r'$M_* / M_\odot$')
        for ax in self.topaxes:    
            ax.set_xlim([0.0, 0.34])
            ax.set_ylim([7, 12])
            ax.set_yticks(np.arange(7, 12, 1))
        ...
        self.fig1.savefig('./FIGURES/MS_RED.pdf')
        plt.show()
