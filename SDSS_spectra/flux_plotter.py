...
class Main:
    ...
    def reading_plotting(self):
        ...
        Main.plotter(self, 'HA', 'HA_SDSS', r'$H\alpha \; flux \; GAMA$', [-50, 5000], r'$H\alpha \; flux \; SDSS$', [-50, 1000], './FIGURES/GAMA_FLUXES/HA_FULL.pdf')
        print('Halpha flux plotted')
        #Main.plotter(self, 'HgF_cont', 'Hg_cont_SDSS', r'$H\gamma \; cont. \; GAMA$', [0, 150], r'$H\gamma \; cont. \; SDSS$', [0, 40])
        #Main.plotter(self, 'HgF_cont', 'Hg_cont_SDSS', r'$H\gamma \; cont. \; GAMA$', [0, 150], r'$H\gamma \; cont. \; SDSS$', [0, 40])
...
#cbar_ax = axs[1].add_axes([0.85, 0.15, 0.05, 0.7])
#fig.colorbar(s_m, cax=cbar_ax, ax=axs[1], location='right')

# fig, axs = plt.subplots(1, 2, figsize=(12, 6), tight_layout=True)

# for i, item in enumerate(df['HA']):
#     axs[0].scatter(item/df['HA_er'][i], df['HA_SDSS'][i]/df['HA_er_SDSS'][i], color=cd_WHAN[df['WHAN'][i]][0], alpha = 1, marker='.')
...
#         color = 'black'        
#     axs[1].scatter(item/df['HA_er'][i], df['HA_SDSS'][i]/df['HA_er_SDSS'][i], color=color, alpha = 1, marker='.')
...
# axes = [axs[0], axs[1]]
# for ax in axes:
#     ax.plot([0, 200], [0, 200], c='red')
#     ax.set_xlabel(r'$SN H\alpha \; GAMA$')
    
# gama_x = np.array(df['HA']/df['HA_er'])
